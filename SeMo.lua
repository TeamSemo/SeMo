...
if not f then   
if not redis:get(Server_SeMo.."Token_DevSeMo") then
...
redis:set(Server_SeMo.."Token_DevSeMo",token)
end 
else
...
------------------------------------------------------------------------------------------------------------
local DevSeMo_Info_Sudo = io.open("Info_Sudo.lua", 'w')
DevSeMo_Info_Sudo:write([[
do 
local SeMo_INFO = {
Id_DevSeMo = ]]..redis:get(Server_SeMo.."Id_DevSeMo")..[[,
UserName_SeMo = "]]..redis:get(Server_SeMo.."User_DevSeMo1")..[[",
Token_Bot = "]]..redis:get(Server_SeMo.."Token_DevSeMo")..[["
}
return SeMo_INFO
end

]])
DevSeMo_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_SeMo = io.open("SeMo", 'w')
Run_File_SeMo:write([[
#!/usr/bin/env bash
cd $HOME/SeMo
token="]]..redis:get(Server_SeMo.."Token_DevSeMo")..[["
while(true) do
rm -fr ../.telegram-cli
...
os.execute('chmod +x SeMo')
os.execute('chmod +x Run')
os.execute('./Run')
Status = true
else   
f:close()  
redis:del(Server_SeMo.."Token_DevSeMo");redis:del(Server_SeMo.."Id_DevSeMo");redis:del(Server_SeMo.."User_DevSeMo1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_SeMo
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DevSeMo
Ids_Dev = {sudos.Id_DevSeMo,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "سيمو"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_SeMo(msg)  
local Dev_SeMo = false  
...
local function Dev_SeMo_User(user)  
local Dev_SeMo_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_SeMo_User = true  
end  
end  
return Dev_SeMo_User  
end 
...
if Dev_SeMo_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/TeamSemo/SeMo/master/Script.lua'
...
url = 'https://raw.githubusercontent.com/TeamSemo/SeMo/master/SeMo.lua'
file_path = 'SeMo.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
...
data.first_name_ = gmatch or 'SeMo'
end
...
------------------------------------------------------------------------------------------------------------
function FilesSeMo(msg)
File_Bot = dofile("Script.lua")
if File_Bot.SeMo and msg then
Text_File = File_Bot.SeMo(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesSeMoBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.SeMoFile and msg then
Text_FileBot = Text_FileBot.SeMoFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_SeMo_File(msg,data)
if msg then
msg = data.message_
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:DevSeMo'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:DevSeMo'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:DevSeMo',text)
redis:del(bot_id..'GetTexting:DevSeMo'..msg.chat_id_..':'..msg.sender_user_id_)
...
if Dev_SeMo(msg) then
if text == 'نقل الاحصائيات' then
local Users = redis:smembers(bot_id.."SeMo:UsersBot")
local Groups = redis:smembers(bot_id..'SeMo:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'SeMo:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:Group"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'SeMo:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'SeMo:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'SeMo:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/TeamSemo/SeMo/master/SeMo.lua','SeMo.lua') 
download_to_file('https://raw.githubusercontent.com/TeamSemo/SeMo/master/Script.lua','Script.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/TeamSemo/Files_SeMo/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌔︙قائمه ملفات متجر سورس SeMo\n⌔︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/TeamSemo/Files_SeMo/master/Files_SeMo/"..File_Name)
if Res == 200 then
...
if text == '/start' then  
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
...
if not Dev_SeMo(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:DevSeMo'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:DevSeMo')
...
if Dev_SeMo_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..SeMo..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_SeMo(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if text and text:match("^اضف مطور @(.*)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if text and text:match("^حذف مطور @(.*)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if text and text:match("^اضف مطور (%d+)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if text and text:match("^حذف مطور (%d+)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if text == 'جلب نسخه احتياطيه' and Dev_SeMo(msg) or text == 'جلب نسخه الكروبات' and Dev_SeMo(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
...
if text == ("مسح قائمه العام") and Dev_SeMo(msg) or text == ("مسح المحظورين عام") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
elseif text == ("قائمه العام") and Dev_SeMo(msg) or text == ("المحظورين عام") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if Dev_SeMo_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
if Dev_SeMo_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_SeMo(msg) then
local url,res = http.request('https://hso.mohammed-api.com/ApI_SeMo.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.SeMo ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Seemoo6 }')   
...
elseif text == "تفعيل المغادره" and Dev_SeMo(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_SeMo(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_SeMo(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_SeMo(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_SeMo(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_SeMo(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_SeMo(msg) then
redis:set(bot_id..'GetTexting:DevSeMo'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_SeMo(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_SeMo(msg) then 
...
elseif text == ("ردود المطور") and Dev_SeMo(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_SeMo(msg) then 
...
elseif text == "حذف رد للكل" and Dev_SeMo(msg) then 
...
if Dev_SeMo_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
redis:srem(bot_id.."President:Group"..msg.chat_id_,result.id_)
redis:srem(bot_id..'Constructor:Group'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Manager:Group'..msg.chat_id_, result.id_)
...
elseif text == "تغير اسم البوت" and Dev_SeMo(msg) or text == "تغيير اسم البوت" and Dev_SeMo(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_SeMo(msg) then 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_SeMo(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_SeMo(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_SeMo(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevSeMo = redis:get(bot_id..'Texting:DevSeMo')
if TextingDevSeMo then 
send(msg.chat_id_, msg.id_,TextingDevSeMo)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_SeMo(msg) then
redis:del(bot_id..'Texting:DevSeMo')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_SeMo(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_SeMo_File(msg,data)
FilesSeMoBot(msg,data)
